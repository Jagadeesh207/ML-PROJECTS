...
# @st.cache_resource is the correct decorator for non-data objects like models
@st.cache_resource
def load_artifacts():
    """
    Loads the saved model, scaler, and one-hot encoder from pickle files
    using paths relative to the script's location.
    """
    # Get the directory of the current script
    script_dir = Path(__file__).parent

    # Construct the full paths to the artifact files
    model_path = script_dir / 'model.pkl'
    scaler_path = script_dir / 'scaler.pkl'
    onehot_path = script_dir / 'onehot.pkl' # Make sure this filename is correct in your repo

    # Open files using their full paths
    with open(model_path, 'rb') as model_file:
        model = pickle.load(model_file)
    with open(scaler_path, 'rb') as scaler_file:
        scaler = pickle.load(scaler_file)
    with open(onehot_path, 'rb') as onehot_file:
